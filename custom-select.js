document.addEventListener('DOMContentLoaded', function () {
	...
	if (selectElements.length > 0) {
			...
			const renderOptions = (searchText, container) => {
				...
				container.innerHTML = filteredOptions
					.map((element, index) => {
						...
						return `<span class="custom-select-option-item ${isDisabled}" data-value="${
							element.value
						}" ${getAttributesAsString(element)} data-index="${index}">${
							element.innerText
						}</span>`;
					})
					.join('');
		...
		`;
			...
			const dropdownContainer = customSelect.querySelector(
				'.custom-select-dropdown-container'
			);
			const dropdown = customSelect.querySelector('.custom-select-dropdown');
			const optionContainer = customSelect.querySelector(
			...
			customSelect.classList.add(
				...Array.from(selectElement.classList).filter(
					(d) => d != 'custom-select'
				)
			);
			...
			const updateActiveOption = (direction) => {
				const items = optionContainer.querySelectorAll(
					'.custom-select-option-item:not(.disabled)'
				);
			...
			customSelect.addEventListener('focus', () => {
				document
					.querySelectorAll('.custom-select-dropdown-container')
					.forEach((d) => d.classList.remove('custom-select-active'));
